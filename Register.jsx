...
// Formulaire de création de compte avec choix du rôle :
// Voyageur (cherche des voyages) ou Prestataire (propose des offres).
// ============================================================

import { useState } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { motion } from 'framer-motion';
import { Mail, Lock, Eye, EyeOff, Plane, User, ArrowRight, Building2 } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { useToast } from '../context/ToastContext';

export default function Register() {
  // État du formulaire avec les noms de champs français (correspond aux colonnes de la BDD)
  const [form, setForm]       = useState({ prenom: '', nom: '', email: '', password: '', role: 'voyageur' });
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div className="input-group">
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input value={form.prenom} onChange={e => setForm(p => ({ ...p, prenom: e.target.value }))} className="input-field" style={{ width: '100%', paddingLeft: 36 }} placeholder="Jean" required />
                </div>
              </div>
              <div className="input-group">
                <label className="input-label">Nom</label>
                <input value={form.nom} onChange={e => setForm(p => ({ ...p, nom: e.target.value }))} className="input-field" style={{ width: '100%' }} placeholder="Dupont" required />
              </div>
            </div>
            ...
            <div className="input-group">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                {/* Options de rôle avec icônes Lucide */}
              {[
                  { value: 'voyageur',    Icone: Plane,     label: 'Voyageur',    desc: 'Je cherche des voyages' },
                  { value: 'prestataire', Icone: Building2, label: 'Prestataire', desc: 'Je propose des offres' },
                ].map(opt => (
                  <label key={opt.value} style={{ padding: '12px 14px', borderRadius: 12, border: `1px solid ${form.role === opt.value ? 'rgba(201,168,76,0.4)' : 'var(--border-light)'}`, background: form.role === opt.value ? 'rgba(201,168,76,0.05)' : 'var(--dark-4)', cursor: 'pointer', transition: 'all 0.2s' }}>
                    ...
                    <p style={{ fontWeight: 600, fontSize: '0.85rem', marginBottom: 2, color: form.role === opt.value ? '#C9A84C' : 'var(--text)', display: 'flex', alignItems: 'center', gap: 6 }}>
                      <opt.Icone size={13} style={{ transform: opt.value === 'traveler' ? 'rotate(-30deg)' : 'none' }} />
                      {opt.label}
                    </p>
