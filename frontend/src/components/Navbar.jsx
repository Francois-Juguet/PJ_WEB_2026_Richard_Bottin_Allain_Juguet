...
// Transparente sur la page d'accueil, opaque sur les autres.
// Gère le menu mobile, le menu utilisateur et le badge panier.
// ============================================================
...
export default function Navbar() {
  ...
  // Sur la page d'accueil, la navbar commence transparente et devient opaque au scroll
  ...
  useEffect(() => {
    if (user) {
      notificationsAPI.getAll().then(r => setNotifCount(r.data.nombre_non_lues)).catch(() => {});
    }
  }, [user, location]);

  // Déconnexion : ferme le menu puis redirige vers l'accueil
            ...
            <>
              ...
              <div style={{ position: 'relative' }}>
                <button onClick={() => setUserMenu(p => !p)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 12px', borderRadius: 10, background: 'rgba(201,168,76,0.1)', border: '1px solid rgba(201,168,76,0.2)', cursor: 'pointer', color: '#E8E8F0' }}>
                  ...
                  <span style={{ fontSize: '0.85rem', fontWeight: 500 }}>{user.prenom}</span>
                  ...
                  {userMenu && (
                    <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} transition={{ duration: 0.15 }}
                      ...
                      <div style={{ padding: '8px 12px 12px', borderBottom: '1px solid rgba(255,255,255,0.06)', marginBottom: 8 }}>
                        <p style={{ fontSize: '0.85rem', fontWeight: 600 }}>{user.prenom} {user.nom}</p>
                      ...
                      {[
                        ...
                        ...(user.role === 'prestataire' || user.role === 'admin' ? [{ to: '/provider', label: 'Espace prestataire', icon: Settings }] : []),
