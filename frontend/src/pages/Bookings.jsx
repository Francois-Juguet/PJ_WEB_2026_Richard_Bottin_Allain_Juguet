// ============================================================
// Bookings.jsx — historique des réservations de l'utilisateur
// Affiche chaque réservation avec son statut, son prix total
// et son statut de paiement. Lien vers l'itinéraire associé.
// ============================================================

import { useState, useEffect } from 'react';
import { motion } from 'framer-motion';
import { ShoppingBag, MapPin, Check, Eye } from 'lucide-react';
...
export default function Bookings() {
  ...
  // Configuration visuelle de chaque statut de réservation (couleur + libellé)
  const statusConfig = {
    confirme:   { color: '#4ade80', bg: 'rgba(74,222,128,0.1)', label: 'Confirmée' },
    en_attente: { color: '#C9A84C', bg: 'rgba(201,168,76,0.1)', label: 'En attente' },
    annule:     { color: '#f87171', bg: 'rgba(239,68,68,0.1)', label: 'Annulée' },
    termine:    { color: '#60a5fa', bg: 'rgba(96,165,250,0.1)', label: 'Terminée' },
  };
      ...
      <div className="container section">
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 800 }}>
            {bookings.map((b, i) => {
              const sc = statusConfig[b.statut] || statusConfig.en_attente;
              return (
                <motion.div key={b.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: '0.75rem', color: 'var(--text-dim)' }}>
                      Réf: <span style={{ color: '#C9A84C', fontWeight: 600 }}>{b.reference}</span> · {new Date(b.cree_le).toLocaleDateString('fr-FR')}
                    </p>
                  </div>

                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <p style={{ fontSize: '1.4rem', fontWeight: 700, color: '#C9A84C', fontFamily: 'Playfair Display' }}>
                      {Number(b.prix_total).toLocaleString('fr-FR')} €
                    </p>
                    <p style={{ fontSize: '0.75rem', color: b.statut_paiement === 'paye' ? '#4ade80' : 'var(--text-dim)', display: 'flex', alignItems: 'center', gap: 4, justifyContent: 'flex-end' }}>
                      {b.statut_paiement === 'paye' ? <><Check size={12} style={{ strokeWidth: 3 }} /> Payé</> : 'En attente'}
                    </p>
