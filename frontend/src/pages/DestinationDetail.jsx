...
export default function DestinationDetail() {
  ...
  const startTrip = async () => {
    ...
    try {
      const { data } = await itinerariesAPI.create({ title: `Voyage à ${dest.nom}`, destination_id: dest.id });
  ...
  return (
    <div className="page" style={{ paddingTop: 70 }}>
      ...
      <div style={{ position: 'relative', height: '55vh', minHeight: 400, overflow: 'hidden' }}>
        <img src={dest.image} alt={dest.nom} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        ...
        <div className="container" style={{ position: 'absolute', inset: '0 0 0 0', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', paddingBottom: 40 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            ...
            <span style={{ color: '#C9A84C', fontSize: '0.9rem' }}>{dest.pays}</span>
            {dest.region && <><span style={{ color: '#5A5A78' }}>·</span><span style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>{dest.region}</span></>}
          </div>
          <h1 style={{ marginBottom: 12 }}>{dest.nom}</h1>
          <div style={{ display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
            <StarRating rating={dest.note} count={dest.nombre_avis} size={15} />
            <span className="badge badge-gold">{dest.categorie}</span>
            <span style={{ color: '#C9A84C', fontWeight: 700, fontSize: '1.1rem' }}>À partir de {Number(dest.prix_depuis).toLocaleString('fr-FR')} €</span>
          </div>
        </div>
      </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 40, alignItems: 'start' }}>
              ...
              <div style={{ background: 'var(--dark-3)', border: '1px solid var(--border)', borderRadius: 20, padding: 28, position: 'sticky', top: 90 }}>
                <div style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ fontSize: '2rem', fontWeight: 700, color: '#C9A84C', fontFamily: 'Playfair Display' }}>{Number(dest.prix_depuis).toLocaleString('fr-FR')} €</div>
                ...
                {dest.accommodations?.map(acc => (
                  <div key={acc.id} className="card" style={{ overflow: 'hidden', cursor: 'pointer' }}
                    ...
                    <div style={{ height: 180, background: 'var(--dark-4)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      {acc.images ? <img src={JSON.parse(acc.images || '[]')[0]} alt={acc.nom} style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={e => e.target.style.display = 'none'} /> : <Hotel size={40} style={{ color: 'var(--text-dim)' }} />}
                    </div>
                    <div style={{ padding: 20 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: 8 }}>
                        <h3 style={{ fontSize: '1rem', fontFamily: 'Playfair Display' }}>{acc.nom}</h3>
                        <span style={{ color: '#C9A84C', fontWeight: 700, whiteSpace: 'nowrap', marginLeft: 8 }}>{Number(acc.prix_nuit).toLocaleString('fr-FR')} €/nuit</span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                        <span className="badge badge-blue">{acc.type}</span>
                        <span style={{ color: '#C9A84C', fontSize: '0.8rem' }}>{'★'.repeat(acc.etoiles || 0)}</span>
                      </div>
                ...
                {dest.activities?.map(act => (
                  <div key={act.id} className="card" style={{ overflow: 'hidden', cursor: 'pointer' }}
                    ...
                    <div style={{ height: 160, background: 'var(--dark-4)', overflow: 'hidden' }}>
                      {act.image && <img src={act.image} alt={act.nom} style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={e => e.target.style.display = 'none'} />}
                    </div>
                    <div style={{ padding: 16 }}>
                      <span className="badge badge-green" style={{ marginBottom: 8, fontSize: '0.7rem' }}>{act.categorie}</span>
                      <h3 style={{ fontSize: '0.95rem', fontFamily: 'Playfair Display', marginBottom: 8 }}>{act.nom}</h3>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--text-muted)', fontSize: '0.8rem' }}>
                          <Clock size={12} />
                          {act.duree_heures}h
                        </div>
                        <span style={{ color: '#C9A84C', fontWeight: 700 }}>{Number(act.prix).toLocaleString('fr-FR')} €</span>
                      </div>
                    </div>
                  </div>
                ))}
                ...
                {dest.reviews?.map(review => (
                  <div key={review.id} style={{ background: 'var(--dark-3)', border: '1px solid var(--border-light)', borderRadius: 16, padding: 20 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                      ...
                      <div>
                        <p style={{ fontWeight: 600, fontSize: '0.9rem' }}>{review.prenom} {review.nom}</p>
                        <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{new Date(review.cree_le).toLocaleDateString('fr-FR')}</p>
                      </div>
