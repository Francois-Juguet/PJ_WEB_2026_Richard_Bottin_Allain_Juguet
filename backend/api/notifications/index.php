...
if ($method === 'GET') {
    ...
    if ($action === 'mark_all_read') {
        ...
        $db->prepare('UPDATE notifications SET is_read = 1 WHERE id = ? AND user_id = ?')->execute([(int)$body['id'], $user['id']]);
    ...
    $body = json_decode(file_get_contents('php://input'), true);
    if (!empty($body['id'])) {
        $db->prepare('DELETE FROM notifications WHERE id = ? AND user_id = ?')->execute([(int)$body['id'], $user['id']]);
    } else {
