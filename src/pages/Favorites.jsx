...
const TYPE_CONFIG = {
  ...
  accommodation: { label: 'Hébergement',  color: '#a78bfa', icon: Hotel,    bg: 'rgba(167,139,250,0.12)', link: () => `/accommodations` },
  activity:      { label: 'Activité',     color: '#4ade80', icon: Compass,  bg: 'rgba(74,222,128,0.12)',  link: () => `/activities` },
};

const TABS = [
  { key: 'all',           label: 'Tous' },
  { key: 'destination',   label: 'Destinations' },
  { key: 'accommodation', label: 'Hébergements' },
...
export default function Favorites() {
  ...
  const remove = async (fav) => {
    try {
      await usersAPI.toggleFavorite({ entity_type: fav.entity_type, entity_id: fav.entity_id });
      setFavorites(p => p.filter(f => f.fav_id !== fav.fav_id));
      toast('Retiré des favoris', 'success');
    } catch { toast('Erreur', 'error'); }
  };

  const filtered = filter === 'all' ? favorites : favorites.filter(f => f.entity_type === filter);
  const countOf  = key => key === 'all' ? favorites.length : favorites.filter(f => f.entity_type === key).length;
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            ...
            <p style={{ color: 'var(--text-dim)', fontSize: '0.875rem', marginBottom: 28 }}>
              Cliquez sur ❤️ sur les hébergements ou activités pour les sauvegarder ici.
            </p>
          ...
          <>
            ...
            <div className="grid-3">
              {filtered.map((fav, i) => {
                const cfg  = TYPE_CONFIG[fav.entity_type] || TYPE_CONFIG.destination;
                const Icon = cfg.icon;
                return (
                  <motion.div key={fav.fav_id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                    className="card" style={{ overflow: 'hidden' }}>

                    {/* Image */}
                    <div style={{ height: 165, background: 'var(--dark-4)', position: 'relative', overflow: 'hidden' }}>
                      {fav.image
                        ? <img src={fav.image} alt={fav.item_name} style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.4s' }}
                    ...
                    <div style={{ padding: '16px 20px 20px' }}>
                      ...
                      {fav.location && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 10 }}>
                          ...
                          <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{fav.location}</span>
                        </div>
                      )}

                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                        {fav.rating ? <StarRating rating={fav.rating} size={11} /> : <span />}
                        {fav.price && (
                          <span style={{ fontSize: '1rem', fontWeight: 700, color: '#C9A84C' }}>
                            {Number(fav.price).toLocaleString('fr-FR')} €
                          </span>
                        )}
                      </div>

                      <button onClick={() => navigate(cfg.link(fav.entity_id))} className="btn btn-ghost"
