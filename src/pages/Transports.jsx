...
function TransportCard({ t, onBook }) {
  ...
  const occ  = Math.round(((t.total_seats - t.available_seats) / t.total_seats) * 100);
    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 16, minWidth: 280 }}>
        <div style={{ textAlign: 'center' }}>
          <p style={{ fontSize: '1.5rem', fontWeight: 700, fontFamily: 'Playfair Display', lineHeight: 1 }}>{formatTime(t.departure_time)}</p>
          <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: 4 }}>{t.departure_city}</p>
          {t.departure_code && <p style={{ fontSize: '0.7rem', color: 'var(--text-dim)' }}>{t.departure_code}</p>}
        </div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
          ...
          <span style={{ fontSize: '0.7rem', color: t.is_direct ? '#4ade80' : 'var(--text-dim)' }}>
            {t.is_direct ? 'Direct' : `${t.stops} escale${t.stops > 1 ? 's' : ''}`}
          </span>
        </div>

        <div style={{ textAlign: 'center' }}>
          <p style={{ fontSize: '1.5rem', fontWeight: 700, fontFamily: 'Playfair Display', lineHeight: 1 }}>{formatTime(t.arrival_time)}</p>
          <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: 4 }}>{t.arrival_city}</p>
          {t.arrival_code && <p style={{ fontSize: '0.7rem', color: 'var(--text-dim)' }}>{t.arrival_code}</p>}
        </div>
      </div>

      {/* Class */}
      <div style={{ textAlign: 'center', minWidth: 80 }}>
        <span className="badge" style={{ background: t.class_type === 'first' ? 'rgba(201,168,76,0.2)' : t.class_type === 'business' ? 'rgba(99,102,241,0.2)' : 'rgba(255,255,255,0.05)', color: t.class_type === 'first' ? '#C9A84C' : t.class_type === 'business' ? '#818cf8' : 'var(--text-muted)', fontSize: '0.7rem' }}>
          {t.class_type === 'first' ? 'Première' : t.class_type === 'business' ? 'Business' : 'Économique'}
        </span>
        <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: 6 }}>
          <Users size={11} style={{ display: 'inline', marginRight: 3 }} />
          {t.available_seats} places
        </p>
      </div>

      {/* Price & action */}
      <div style={{ textAlign: 'right', minWidth: 120 }}>
        <p style={{ fontSize: '1.6rem', fontWeight: 700, color: '#C9A84C', fontFamily: 'Playfair Display', lineHeight: 1 }}>{Number(t.price).toLocaleString('fr-FR')} €</p>
  ...
  useEffect(() => {
    if (user) {
      itinerariesAPI.getAll().then(r => {
        setItineraries((r.data ?? []).filter(i => i.status === 'draft'));
      }).catch(() => {});
    }
  }, [user]);
        ...
        <div style={{ background: 'rgba(201,168,76,0.07)', borderBottom: '1px solid rgba(201,168,76,0.2)', padding: '10px 0' }}>
          <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            <span style={{ fontSize: '0.875rem', color: '#C9A84C' }}>
              🗺️ Transports {destCity ? `vers ${destCity}` : ''} — filtrés pour votre itinéraire
            </span>
        ...
        <div className="modal-overlay" onClick={() => setShowItinModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', marginBottom: 24 }}>
              {pendingTransport && `${pendingTransport.company} — ${pendingTransport.departure_city} → ${pendingTransport.arrival_city}`}
            </p>
                  ...
                  {itineraries.map(it => (
                    <label key={it.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', borderRadius: 12, border: `1px solid ${selectedItinerary == it.id ? 'rgba(201,168,76,0.4)' : 'var(--border-light)'}`, background: selectedItinerary == it.id ? 'rgba(201,168,76,0.05)' : 'var(--dark-4)', cursor: 'pointer' }}>
                      ...
                      <div>
                        ...
                        {it.destination_name && <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>📍 {it.destination_name}</p>}
                      </div>
                    </label>
                  ))}
                </div>
