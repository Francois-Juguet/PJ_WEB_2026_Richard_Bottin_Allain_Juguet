...
import { Plus, Edit2, Trash2, Hotel, Zap, Plane, X, Save, BarChart2, Package, Star, TrendingUp } from 'lucide-react';
...
const emptyAcc = { name: '', type: 'hotel', description: '', address: '', price_per_night: '', max_capacity: 2, stars: 3, destination_id: '', amenities: [], images: [] };
const emptyAct = { name: '', category: 'tour', description: '', price: '', duration_hours: 2, max_participants: 10, destination_id: '', included: [], image: '' };
const emptyTrans = { type: 'flight', company: '', departure_city: '', arrival_city: '', departure_code: '', arrival_code: '', departure_time: '', arrival_time: '', price: '', available_seats: 100, total_seats: 180, class_type: 'economy', is_direct: true, stops: 0 };
      ...
      <div className="container" style={{ padding: '32px 24px 80px' }}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            <p style={{ fontSize: '3rem', marginBottom: 16 }}>{tab === 'accommodations' ? '🏨' : tab === 'activities' ? '🎯' : '✈️'}</p>
              ...
              <motion.div key={item.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                ...
                <div style={{ height: 130, background: 'var(--dark-4)', position: 'relative', overflow: 'hidden' }}>
                  ...
                  <div style={{ position: 'absolute', top: 10, right: 10, display: 'flex', gap: 6 }}>
                    ...
                    {tab !== 'transports' && (
                      <button onClick={() => setDelConfirm({ id: item.id, name: item.name, type: tab })}
                  ...
                  <span style={{ position: 'absolute', bottom: 10, left: 10, background: 'rgba(0,0,0,0.7)', padding: '3px 10px', borderRadius: 8, fontSize: '0.72rem', color: '#C9A84C', fontWeight: 600 }}>
                    {item.type || item.category || item.class_type}
                  </span>
                </div>

                <div style={{ padding: 16 }}>
                  <h3 style={{ fontSize: '1rem', fontFamily: 'Playfair Display', fontWeight: 700, marginBottom: 6, lineHeight: 1.3 }}>{item.name || `${item.departure_city} → ${item.arrival_city}`}</h3>
                  {item.destination_name && <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: 8 }}>📍 {item.destination_name}</p>}
                  {item.departure_city && <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: 8 }}>✈️ {item.company}</p>}

                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ color: '#C9A84C', fontWeight: 700, fontSize: '1rem' }}>
                      {Number(item.price_per_night || item.price || item.price).toLocaleString('fr-FR')} €
                      <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', fontWeight: 400 }}>
                        {item.price_per_night ? '/nuit' : item.duration_hours ? '/pers.' : ''}
                      </span>
                    </span>
                    {item.rating > 0 && <StarRating rating={item.rating} count={item.reviews_count} size={11} />}
                  </div>
                </div>
              </motion.div>
            ))}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                {modal.type === 'accommodations' && (<>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <div className="input-group">
                      ...
                      <input value={form.name || ''} onChange={e => set('name', e.target.value)} className="input-field" placeholder="Villa Paradiso" />
                    </div>
                    <div className="input-group">
                      <label className="input-label">Type *</label>
                      <select value={form.type || 'hotel'} onChange={e => set('type', e.target.value)} className="input-field">
                        {['hotel','villa','apartment','hostel','resort'].map(t => <option key={t} value={t}>{t}</option>)}
                      </select>
                    </div>
                  </div>
                  <div className="input-group">
                  ...
                  <div className="input-group">
                    ...
                    <input value={form.address || ''} onChange={e => set('address', e.target.value)} className="input-field" placeholder="123 rue du Soleil, Bali" />
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                    <div className="input-group">
                      <label className="input-label">Prix/nuit (€) *</label>
                      <input type="number" value={form.price_per_night || ''} onChange={e => set('price_per_night', e.target.value)} className="input-field" placeholder="250" />
                    </div>
                    <div className="input-group">
                      <label className="input-label">Capacité max</label>
                      <input type="number" value={form.max_capacity || 2} onChange={e => set('max_capacity', e.target.value)} className="input-field" />
                    </div>
                    <div className="input-group">
                      <label className="input-label">Étoiles</label>
                      <select value={form.stars || 3} onChange={e => set('stars', e.target.value)} className="input-field">
                  ...
                  <div className="input-group">
                    ...
                    <input value={Array.isArray(form.amenities) ? form.amenities.join(', ') : form.amenities || ''} onChange={e => set('amenities', e.target.value.split(',').map(s => s.trim()).filter(Boolean))} className="input-field" placeholder="Piscine, Spa, WiFi, Restaurant..." />
                  </div>
                </>)}
                ...
                {modal.type === 'activities' && (<>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <div className="input-group">
                      <label className="input-label">Nom *</label>
                      <input value={form.name || ''} onChange={e => set('name', e.target.value)} className="input-field" placeholder="Plongée sous-marine" />
                    </div>
                    <div className="input-group">
                      <label className="input-label">Catégorie *</label>
                      <select value={form.category || 'tour'} onChange={e => set('category', e.target.value)} className="input-field">
                        {['adventure','culture','nature','wellness','gastronomy','family','tour'].map(c => <option key={c} value={c}>{c}</option>)}
                      </select>
                    </div>
                  </div>
                  <div className="input-group">
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                    <div className="input-group">
                      ...
                      <input type="number" value={form.price || ''} onChange={e => set('price', e.target.value)} className="input-field" placeholder="95" />
                    </div>
                    <div className="input-group">
                      <label className="input-label">Durée (heures)</label>
                      <input type="number" step="0.5" value={form.duration_hours || 2} onChange={e => set('duration_hours', e.target.value)} className="input-field" />
                    </div>
                    <div className="input-group">
                      <label className="input-label">Participants max</label>
                      <input type="number" value={form.max_participants || 10} onChange={e => set('max_participants', e.target.value)} className="input-field" />
                    </div>
                  </div>
                  <div className="input-group">
                  ...
                  <div className="input-group">
                    ...
                    <input value={Array.isArray(form.included) ? form.included.join(', ') : form.included || ''} onChange={e => set('included', e.target.value.split(',').map(s => s.trim()).filter(Boolean))} className="input-field" placeholder="Équipement, Guide, Assurance..." />
                  </div>
                </>)}
                ...
                {modal.type === 'transports' && (<>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <div className="input-group">
                      <label className="input-label">Type *</label>
                      <select value={form.type || 'flight'} onChange={e => set('type', e.target.value)} className="input-field">
                        {['flight','train','bus','car','ferry'].map(t => <option key={t} value={t}>{t}</option>)}
                      </select>
                    </div>
                    <div className="input-group">
                      <label className="input-label">Compagnie *</label>
                      <input value={form.company || ''} onChange={e => set('company', e.target.value)} className="input-field" placeholder="Air France" />
                    </div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <div className="input-group">
                      <label className="input-label">Ville départ *</label>
                      <input value={form.departure_city || ''} onChange={e => set('departure_city', e.target.value)} className="input-field" placeholder="Paris" />
                    </div>
                    <div className="input-group">
                      <label className="input-label">Ville arrivée *</label>
                      <input value={form.arrival_city || ''} onChange={e => set('arrival_city', e.target.value)} className="input-field" placeholder="Dubaï" />
                    </div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <div className="input-group">
                      <label className="input-label">Code départ</label>
                      <input value={form.departure_code || ''} onChange={e => set('departure_code', e.target.value)} className="input-field" placeholder="CDG" />
                    </div>
                    <div className="input-group">
                      <label className="input-label">Code arrivée</label>
                      <input value={form.arrival_code || ''} onChange={e => set('arrival_code', e.target.value)} className="input-field" placeholder="DXB" />
                    </div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <div className="input-group">
                      <label className="input-label">Départ *</label>
                      <input type="datetime-local" value={form.departure_time || ''} onChange={e => set('departure_time', e.target.value)} className="input-field" />
                    </div>
                    <div className="input-group">
                      <label className="input-label">Arrivée *</label>
                      <input type="datetime-local" value={form.arrival_time || ''} onChange={e => set('arrival_time', e.target.value)} className="input-field" />
                    </div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                    <div className="input-group">
                      <label className="input-label">Prix (€) *</label>
                      <input type="number" value={form.price || ''} onChange={e => set('price', e.target.value)} className="input-field" placeholder="350" />
                    </div>
                    <div className="input-group">
                      <label className="input-label">Places dispo</label>
                      <input type="number" value={form.available_seats || 100} onChange={e => set('available_seats', e.target.value)} className="input-field" />
                    </div>
                    <div className="input-group">
                      <label className="input-label">Classe</label>
                      <select value={form.class_type || 'economy'} onChange={e => set('class_type', e.target.value)} className="input-field">
