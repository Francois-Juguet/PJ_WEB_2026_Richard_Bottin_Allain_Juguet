...
import { Users, MapPin, ShoppingBag, TrendingUp, Plus, Trash2, Globe, Hotel, Compass, Plane } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const statCards = [
    ...
    { icon: TrendingUp, label: 'Revenus (€)', value: bookings.reduce((s, b) => s + (b.status === 'confirmed' ? Number(b.total_price) : 0), 0).toLocaleString('fr-FR'), color: '#60a5fa', bg: 'rgba(96,165,250,0.1)' },
    { icon: Users, label: 'Voyageurs actifs', value: [...new Set(bookings.map(b => b.user_id))].length, color: '#f472b6', bg: 'rgba(244,114,182,0.1)' },
  ];
                    ...
                    <tbody>
                      {bookings.slice(0, 8).map((b, i) => (
                        <tr key={b.id} style={{ borderBottom: i < bookings.length - 1 ? '1px solid var(--border-light)' : 'none' }}>
                          <td style={{ padding: '14px 20px', fontSize: '0.85rem', color: '#C9A84C', fontWeight: 600 }}>{b.booking_reference}</td>
                          <td style={{ padding: '14px 20px', fontSize: '0.875rem' }}>{b.first_name} {b.last_name}</td>
                          <td style={{ padding: '14px 20px', fontSize: '0.875rem', color: 'var(--text-muted)' }}>{b.trip_title}</td>
                          <td style={{ padding: '14px 20px', fontSize: '0.875rem', fontWeight: 600 }}>{Number(b.total_price).toLocaleString('fr-FR')} €</td>
                          <td style={{ padding: '14px 20px' }}>
                            <span style={{ padding: '3px 10px', borderRadius: 100, fontSize: '0.75rem', fontWeight: 600, background: b.status === 'confirmed' ? 'rgba(74,222,128,0.1)' : 'rgba(201,168,76,0.1)', color: b.status === 'confirmed' ? '#4ade80' : '#C9A84C' }}>
                    ...
                    <tbody>
                      {destinations.map((d, i) => (
                        <tr key={d.id} style={{ borderBottom: i < destinations.length - 1 ? '1px solid var(--border-light)' : 'none' }}>
                          ...
                          <td style={{ padding: '12px 16px', fontSize: '0.875rem', color: 'var(--text-muted)' }}>{d.country}</td>
                          <td style={{ padding: '12px 16px' }}><span className="badge badge-gold" style={{ fontSize: '0.7rem' }}>{d.category}</span></td>
                          <td style={{ padding: '12px 16px', fontSize: '0.875rem', color: '#C9A84C', fontWeight: 600 }}>{Number(d.price_from).toLocaleString('fr-FR')} €</td>
                          <td style={{ padding: '12px 16px', fontSize: '0.875rem' }}>⭐ {Number(d.rating).toFixed(1)}</td>
                          <td style={{ padding: '12px 16px', fontSize: '0.875rem' }}>{d.is_featured ? '✅' : '—'}</td>
                          <td style={{ padding: '12px 16px' }}>
                ...
                <div style={{ background: 'var(--dark-3)', border: '1px solid var(--border-light)', borderRadius: 20, overflow: 'hidden' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    ...
                    <tbody>
                      {bookings.map((b, i) => (
                        <tr key={b.id} style={{ borderBottom: i < bookings.length - 1 ? '1px solid var(--border-light)' : 'none' }}>
                          <td style={{ padding: '12px 16px', fontSize: '0.85rem', color: '#C9A84C', fontWeight: 600 }}>{b.booking_reference}</td>
                          <td style={{ padding: '12px 16px', fontSize: '0.875rem' }}>{b.first_name} {b.last_name}</td>
                          <td style={{ padding: '12px 16px', fontSize: '0.8rem', color: 'var(--text-muted)' }}>{b.email}</td>
                          <td style={{ padding: '12px 16px', fontSize: '0.875rem', color: 'var(--text-muted)' }}>{b.trip_title}</td>
                          <td style={{ padding: '12px 16px', fontSize: '0.875rem', color: 'var(--text-muted)' }}>{b.destination || '—'}</td>
                          <td style={{ padding: '12px 16px', fontSize: '0.875rem', fontWeight: 600 }}>{Number(b.total_price).toLocaleString('fr-FR')} €</td>
                          <td style={{ padding: '12px 16px' }}>
                            <span style={{ padding: '3px 10px', borderRadius: 100, fontSize: '0.75rem', fontWeight: 600, background: b.status === 'confirmed' ? 'rgba(74,222,128,0.1)' : 'rgba(201,168,76,0.1)', color: b.status === 'confirmed' ? '#4ade80' : '#C9A84C' }}>
                              {b.status}
                            </span>
                          </td>
                          <td style={{ padding: '12px 16px', fontSize: '0.8rem', color: 'var(--text-muted)' }}>{new Date(b.created_at).toLocaleDateString('fr-FR')}</td>
                        </tr>
                      ))}
                      {bookings.length === 0 && (
