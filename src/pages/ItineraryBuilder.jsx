            ...
            <div>
              {itineraries.map(it => (
                <button key={it.id} onClick={() => { setCurrent(it); navigate(`/itinerary/${it.id}`); }}
                  ...
                  <p style={{ fontWeight: 600, fontSize: '0.9rem', color: current?.id === it.id ? '#C9A84C' : 'var(--text)', marginBottom: 4 }}>{it.title}</p>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 6 }}>
                    <span className="badge" style={{ fontSize: '0.65rem', padding: '2px 8px', background: it.status === 'confirmed' ? 'rgba(74,222,128,0.1)' : 'rgba(255,255,255,0.05)', color: it.status === 'confirmed' ? '#4ade80' : 'var(--text-muted)' }}>
                      {it.status === 'confirmed' ? '✓ Confirmé' : 'Brouillon'}
                    </span>
                    {it.total_price > 0 && <span style={{ fontSize: '0.8rem', color: '#C9A84C', fontWeight: 600 }}>{Number(it.total_price).toLocaleString('fr-FR')} €</span>}
                  </div>
                </button>
              ))}
            </div>
        ...
        {current ? (
          <div>
            <div style={{ background: 'var(--dark-3)', borderRadius: 20, border: '1px solid var(--border-light)', padding: 28, marginBottom: 24 }}>
                  ...
                  <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                    ...
                    {current.start_date && (
                      <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.875rem', color: 'var(--text-muted)' }}>
                        ...
                        {new Date(current.start_date).toLocaleDateString('fr-FR')} → {current.end_date && new Date(current.end_date).toLocaleDateString('fr-FR')}
                      </span>
                    )}
                    {current.travelers > 1 && (
                      <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.875rem', color: 'var(--text-muted)' }}>
                        <Users size={14} style={{ color: '#C9A84C' }} /> {current.travelers} voyageurs
                      </span>
                    )}
                  </div>
                </div>
                <span className="badge" style={{ background: current.status === 'confirmed' ? 'rgba(74,222,128,0.1)' : 'rgba(201,168,76,0.1)', color: current.status === 'confirmed' ? '#4ade80' : '#C9A84C', padding: '6px 14px' }}>
                  {current.status === 'confirmed' ? '✓ Confirmé' : 'Brouillon'}
                </span>
              </div>
              ...
              {current.status !== 'confirmed' && (
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                  {[
                    { label: 'Transport', path: `/transports?itinerary_id=${current.id}&dest_city=${encodeURIComponent(current.destination_name||'')}&dest_id=${current.destination_id||''}&start_date=${current.start_date||''}&end_date=${current.end_date||''}&travelers=${current.travelers||1}`, icon: Plane },
                    { label: 'Hébergement', path: `/accommodations?itinerary_id=${current.id}&destination_id=${current.destination_id||''}&dest_name=${encodeURIComponent(current.destination_name||'')}&start_date=${current.start_date||''}&end_date=${current.end_date||''}&travelers=${current.travelers||1}`, icon: Hotel },
                    { label: 'Activité', path: `/activities?itinerary_id=${current.id}&destination_id=${current.destination_id||''}&dest_name=${encodeURIComponent(current.destination_name||'')}&start_date=${current.start_date||''}&end_date=${current.end_date||''}&travelers=${current.travelers||1}`, icon: Compass },
            ...
            <Section title="Transports" icon={Plane} items={current.transports} color="#60a5fa"
              renderItem={t => (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
                  <div>
                    <p style={{ fontWeight: 600, fontSize: '0.9rem' }}>{t.company}</p>
                    <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{t.departure_city} → {t.arrival_city}</p>
                  ...
                  <span style={{ color: '#C9A84C', fontWeight: 700 }}>{Number(t.price || 0).toLocaleString('fr-FR')} €</span>
                </div>
              )}
              onRemove={current.status !== 'confirmed' ? (item) => removeItem('remove_transport', item.id) : null}
            ...
            <Section title="Hébergements" icon={Hotel} items={current.accommodations} color="#a78bfa"
              renderItem={a => (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
                  ...
                  <span style={{ color: '#C9A84C', fontWeight: 700 }}>{Number(a.price || 0).toLocaleString('fr-FR')} €</span>
                </div>
              )}
              onRemove={current.status !== 'confirmed' ? (item) => removeItem('remove_accommodation', item.id) : null}
            ...
            <Section title="Activités" icon={Compass} items={current.activities} color="#4ade80"
              renderItem={a => (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                      <span className="badge" style={{ fontSize: '0.65rem', background: 'rgba(74,222,128,0.1)', color: '#4ade80' }}>{a.category}</span>
              ...
              onRemove={current.status !== 'confirmed' ? (item) => removeItem('remove_activity', item.id) : null}
            ...
            <div style={{ background: 'var(--dark-3)', border: '1px solid var(--border)', borderRadius: 20, padding: 24, marginTop: 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                ...
                <span style={{ fontSize: '2rem', fontWeight: 700, color: '#C9A84C', fontFamily: 'Playfair Display' }}>
                  {Number(current.total_price || 0).toLocaleString('fr-FR')} €
                </span>
              </div>
              {current.status !== 'confirmed' && totalItems > 0 && (
              ...
              {totalItems === 0 && current.status !== 'confirmed' && (
        ...
        <div className="modal-overlay" onClick={() => setDelConfirm(false)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 420, textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: 24 }}>
              "<strong>{current?.title}</strong>" et tous ses éléments seront définitivement supprimés.
            </p>
        ...
        <div className="modal-overlay" onClick={() => setShowCheckout(false)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 480 }}>
            ...
            <div style={{ background: 'rgba(201,168,76,0.05)', border: '1px solid rgba(201,168,76,0.2)', borderRadius: 14, padding: '16px 20px', marginBottom: 24 }}>
              <p style={{ fontWeight: 600, marginBottom: 12, fontFamily: 'Playfair Display' }}>{current.title}</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {current.transports?.map(t => (
                  <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                    <span>✈ {t.company} ({t.departure_city} → {t.arrival_city})</span>
                    <span>{Number(t.price || 0).toLocaleString('fr-FR')} €</span>
                  </div>
                ))}
                {current.accommodations?.map(a => (
                  <div key={a.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                    <span>🏨 {a.name}</span>
                    <span>{Number(a.price || 0).toLocaleString('fr-FR')} €</span>
                  </div>
                ))}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 700, color: '#C9A84C' }}>
                  <span>Total</span>
                  <span>{Number(current.total_price).toLocaleString('fr-FR')} €</span>
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              ...
              <button onClick={checkout} className="btn btn-gold" style={{ flex: 1 }}>
                <Check size={16} /> Confirmer ({Number(current.total_price).toLocaleString('fr-FR')} €)
              </button>
            </div>
          </div>
        </div>
