            ...
            {filtered.map((n, i) => {
              ...
              return (
                <motion.div key={n.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}
                  style={{ background: n.is_read ? 'var(--dark-3)' : 'rgba(201,168,76,0.04)', border: `1px solid ${n.is_read ? 'var(--border-light)' : 'rgba(201,168,76,0.15)'}`, borderRadius: 16, padding: '16px 20px', display: 'flex', alignItems: 'flex-start', gap: 16, cursor: !n.is_read ? 'pointer' : 'default', transition: 'all 0.2s' }}
                  onClick={() => !n.is_read && markRead(n.id)}
                  whileHover={{ borderColor: n.is_read ? 'rgba(255,255,255,0.1)' : 'rgba(201,168,76,0.3)' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      ...
                      {!n.is_read && <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#C9A84C', flexShrink: 0 }} />}
