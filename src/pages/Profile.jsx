...
import { User, Mail, Phone, Edit2, Save, X, ShoppingBag, MapPin, Heart, Bell, LayoutDashboard, Calendar } from 'lucide-react';
...
export default function Profile() {
  ...
  const roleLabel = profile.role === 'admin' ? '👑 Administrateur' : profile.role === 'provider' ? '🏢 Prestataire' : '✈️ Voyageur';
  const roleColor = profile.role === 'admin' ? '#f87171' : profile.role === 'provider' ? '#60a5fa' : '#C9A84C';

  const stats = [
    { icon: ShoppingBag, value: profile.bookings_count ?? 0,    label: 'Réservations',  link: '/bookings',  color: '#4ade80' },
    { icon: MapPin,      value: profile.itineraries_count ?? 0, label: 'Itinéraires',   link: '/itinerary', color: '#60a5fa' },
    { icon: Heart,       value: profile.favorites_count ?? 0,   label: 'Favoris',       link: '/destinations', color: '#f472b6' },
  ];

  const quickLinks = [
    { to: '/destinations', emoji: '🌍', label: 'Destinations',    desc: 'Explorer les offres premium' },
    { to: '/itinerary',    emoji: '🗺️', label: 'Mes itinéraires', desc: 'Créer et gérer mes voyages' },
    { to: '/bookings',     emoji: '🎫', label: 'Réservations',    desc: 'Suivre mes réservations' },
    { to: '/notifications',emoji: '🔔', label: 'Notifications',   desc: 'Mes alertes et messages' },
    ...(profile.role === 'provider' || profile.role === 'admin' ? [
      { to: '/provider', emoji: '🏢', label: 'Espace prestataire', desc: 'Gérer mes offres', highlight: true },
    ] : []),
    ...(profile.role === 'admin' ? [
      { to: '/admin', emoji: '👑', label: 'Administration', desc: 'Gérer la plateforme', highlight: true },
    ] : []),
  ];
        ...
        <div className="container" style={{ position: 'relative' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 24, flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1, minWidth: 200 }}>
              <h1 style={{ fontSize: '1.8rem', marginBottom: 4 }}>{profile.first_name} {profile.last_name}</h1>
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}
            ...
            {editing ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  <div className="input-group">
                    ...
                    <input value={form.first_name} onChange={e => setForm(p => ({ ...p, first_name: e.target.value }))} className="input-field" />
                  </div>
                  <div className="input-group">
                    <label className="input-label">Nom</label>
                    <input value={form.last_name} onChange={e => setForm(p => ({ ...p, last_name: e.target.value }))} className="input-field" />
                  </div>
                </div>
                <div className="input-group">
                  <label className="input-label">Téléphone</label>
                  <input type="tel" value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))} className="input-field" placeholder="+33 6 00 00 00 00" />
                </div>
                <div className="input-group">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {[
                  { icon: User,  label: 'Nom complet',  value: `${profile.first_name} ${profile.last_name}` },
